...
def get_best_solution (end_point, model_list, model_number):
    ...
    best_error=-1
    best_solution=[]
    if not (model_number>=0 and model_number<=Last_model):
    ...
    my_model_range=range(1,model_number+1)
    for i in my_model_range:
...
def test_bootstrapping_approach (test_samples_size, dataMat, model_list, directory):
    model_number = Last_model
    if not os.path.isdir(directory):
        os.mkdir(directory)   
    error_data_file="error_vector_model_" + str(model_number+1) + ".txt"
    ...
    if model_number==0:
        angles_scatter_plot (dataMat,model_list[0],directory,"96")
        angles_hexbin_plot (dataMat,model_list[0],directory,"96")
...
if __name__ == '__main__':
    ...
    # Step 1: load MLP-ANN (models):
    model_list = load_models (directory, model_name_prefix, Last_model+1)
    if len(model_list) == 0:
      print('model_list is empty!!')
      print ("Please, run the scorbot_make_models_inverse_kinematics_sub_working_area_spaces.py")
      sys.exit(-1)
    samples, dataMat = data_set_load (directory, test_data_file_name)
    if len(dataMat) == 0:
    ...
    test_bootstrapping_approach (test_samples_size, dataMat, model_list, directory_model)
