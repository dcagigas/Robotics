...
def angles_scatter_plot (dataMat, model_list, model_number_vector, directory, model_name_prefix):
    ...
    oy = data_input[:,[4]] 
    l = x.shape[0]
    prediction = np.zeros(shape=(l, 4))
    for i in range(l):
      model_i = model_number_vector[i]
      my_best_model = model_list[model_i]
      q1_p,q2_p,q3_p,q4_p = my_best_model([x[i],y[i],z[i],op[i],oy[i]], training=False) #predict
      prediction[i][0] = q1_p
      prediction[i][1] = q2_p
      prediction[i][2] = q3_p
      prediction[i][3] = q4_p
...
def angles_hexbin_plot (dataMat, model_list, model_number_vector, directory, model_name_prefix):
